"""
Scores actual knockout_only results against their locked
predictions_snapshot.json entries -- same per-match metrics as
score_league.py (score_match reused unmodified: a tie decided by
penalties is still, correctly, graded as whatever its own goal-score
says, same reasoning as snapshot_cup.py's module docstring).
"""
import json
import os
import sys
...
def score_and_save(config, base_dir):
    """Load <slug>/knockout_fixtures.json + predictions_snapshot.json,
    score every FINISHED fixture that has a locked snapshot entry, and
    write <slug>/results_accuracy.json: {"matches": [...], "summary": {...}}.
    Fixtures without a snapshot entry are skipped, not fabricated -- same
    discipline as score_league.py."""
...
def main():
    if len(sys.argv) != 2:
        print("usage: python score_copa.py competitions/<slug>.json")
        raise SystemExit(1)
